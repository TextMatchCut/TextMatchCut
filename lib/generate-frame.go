...
func ApplyHorizontalBlur(src image.Image, options EfficientVariableDirectionalBlurOptions) *image.RGBA {
	bounds := src.Bounds()
...
func ApplyDirectionalBlurFeathered(src image.Image, options EfficientVariableDirectionalBlurOptions) *image.RGBA {
	bounds := src.Bounds()
...
func GenerateFrame(frameNum int, config Config, aiSnippets []TextSnippet, fontFiles []string, highlightRadius float64) (image.Image, error) {
	...
	case "directional":
		finalImage = ApplyDirectionalBlurFeathered(img, EfficientVariableDirectionalBlurOptions{
	...
	case "gaussian-no-feather":
		...
		finalImage = ApplyGaussianBlur(img, EfficientVariableBlurOptions{
			CenterX:   config.Width / 2,
			CenterY:   config.Height / 2,
			Radius:    highlightRadius,
	...
	case "gaussian":
		finalImage = ApplyGaussianBlurFeathered(img, EfficientVariableBlurOptions{
			CenterX:   config.Width / 2,
			CenterY:   config.Height / 2,
			Radius:    highlightRadius,
			MaxBlur:   config.BlurRadius,
	...
	case "horizontal":
		...
		finalImage = ApplyHorizontalBlur(img, EfficientVariableDirectionalBlurOptions{
			CenterX: config.Width / 2,
			CenterY: config.Height / 2,
			Radius:  highlightRadius,
			// MaxLength: config.BlurRadius,
...
func GenerateFrameAsBase64(frameNum int, config Config, aiSnippets []TextSnippet, fontFiles []string, highlightRadius float64) (string, error) {
	img, err := GenerateFrame(frameNum, config, aiSnippets, fontFiles, highlightRadius)
	if err != nil {
...
func ApplyGaussianBlur(src image.Image, options EfficientVariableBlurOptions) *image.RGBA {
	bounds := src.Bounds()
...
func ApplyGaussianBlurFeathered(src image.Image, options EfficientVariableBlurOptions) *image.RGBA {
	bounds := src.Bounds()
